...
joint.plugins.TooledModelInterface = {
    ...
    resizeToolMarkup: '<circle class="circle" fill="red" r="11"/><path class= "resize" />',
    ...
    toolsDefaults: {
		...
		'.resizeTool .circle':{ ref: '.body', 'ref-dx':-13.5, 'ref-dy':-63},
        '.resizeTool .resize': {
...
//        As ↑this↑ code is not ES5 valid, I remake it ES5-valid in order to have ng-annotate working
    ...
    renderResizeTool: function () {
		...
		var markup = V(this.model.resizeToolMarkup);
        for(var id in markup)
            resizeContainer.append(markup[id].node);
        //resizeContainer.append(V(this.model.resizeToolMarkup).node);
        //resizeContainer.find('path.resize')[0]
        var modello = this.model;
        var performanceLock = false;
        resizeContainer.children()[0].addEventListener("click", function(e){
            //document.resizeInitialValues = {x: e.pageX, y:e.pageY, model: modello, bbox: $(this).closest(".element").find(".scalable")[0].getBoundingClientRect()};
			modello.remove();

			}, true);
		resizeContainer.children()[1].addEventListener("click", function(e){
            //document.resizeInitialValues = {x: e.pageX, y:e.pageY, model: modello, bbox: $(this).closest(".element").find(".scalable")[0].getBoundingClientRect()};
			modello.remove();
